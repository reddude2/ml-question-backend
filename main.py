...
Version 3.1 - With Exam Mode, Tiers & 4 Roles + Materials Management + Automation Support + Calculator Config
"""
...
from routers import calculator_config  # ← NEW: Calculator config router
...
app = FastAPI(
    ...
    * **Calculator Config** - Dynamic calculator configuration with auto-update system
    ...
    * **Admin Dashboard** - User and question management
    ...
    ## 🔥 New in v3.1 - MATERIALS MANAGEMENT & AUTOMATION & CALCULATOR CONFIG
    
    ### 🧮 Calculator Config System (NEW)
    * **Dynamic Configuration** - Auto-fetch passing grades & bobot values from database
    * **Auto-Update** - Frontend automatically gets new values (24h cache)
    * **Version Control** - Track config changes over years
    * **Multi-Calculator** - Support for STAN, IPDN, STIS, STIN, STMKG, POLRI, CPNS, TNI
...
app.include_router(calculator_config.router)  # ← NEW: Calculator config
...
def read_root():
    ...
    return {
        ...
        "features": [
            ...
            "Calculator config with auto-update",  # ← NEW
        ...
        "endpoints": {
            ...
            "calculator-config": "/api/calculator-config - Dynamic calculator configuration",  # ← NEW
            "auto": "/auto - Automated Input (Scrape/Discovery)",
...
def api_info():
    ...
    return {
        "api": {
            "name": "ML Question System API",
            "version": "3.1.0",
            "description": "Backend API for POLRI & CPNS exam preparation with NEVER REPEAT system, Exam Mode, Materials Management, Calculator Config, and ML Question Generation"
        },
        "features": {
            "session_management": {
            ...
            "calculator_config": {  # ← NEW
                "dynamic_config": "Auto-fetch passing grades & bobot from database",
                "auto_update": "Frontend gets new values automatically (24h cache)",
                "version_control": "Track changes over years",
                "supported_calculators": ["STAN", "IPDN", "STIS", "STIN", "STMKG", "POLRI", "CPNS", "TNI"]
            },
        ...
        "endpoints": {
            ...
            "calculator_config": {  # ← NEW
                "GET /api/calculator-config/{calculator_type}": "Get active calculator config",
                "GET /api/calculator-config": "Get all active configs"
            },
            "materials": {
...
async def startup_event():
    ...
    print("   ✅ Calculator config with auto-update")  # ← NEW
    ...
    print("\n🧮 Calculator Config:")  # ← NEW
    print("   ✅ Dynamic passing grade & bobot values")
    print("   ✅ Auto-update frontend (24h cache)")
    print("   ✅ Support 8 calculators (STAN, IPDN, etc.)")
    print("=" * 70)
    print("✅ API is ready!")
    ...
    print("🧮 Calculator: http://localhost:8000/api/calculator-config")  # ← NEW
